...
const ChatContainer = () => {
    ...
    const handleSendImage = async (e) => {
        ...
        if(!file || !file.type.startsWith("image/")){
            toast.error("Selet an image file");
            return;
        }
  ...
  return selectedUser ? (
        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'} `}>
                        ...
                        <div className='text-center text-xs'>
                            <img src={msg.senderId === authUser._id ? authUser?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon} alt="avatar" className='w-7 rounded-full' />
                            <p className='text-gray-500'>{getTime(msg.createdAt)}</p>                         
                        </div>
                    </div>
                ))}
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                    {/* <img src={assets.smile_icon} alt="smile" className='w-5 cursor-pointer' /> */}
                    <input onChange={(e) => setInput(e.target.value)} value={input} onKeyDown={(e)=> e.key === "Enter" ? handleSendMesage(e) : null} type="text" className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400' placeholder='Message...' />
    ...
    <>
        <div className='flex flex-col justify-center items-center text-gray-500 gap-2 bg-white/10 max-md:hidden '>
            <img src={assets.logotexts_med} alt="logo" className='max-w-46' />
